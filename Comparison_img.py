...
def uploaded_file(uploaded_file):
    ...
    if uploaded_file is not None:
        ...
        # st.image(image, use_column_width=True)
        # pil_image = Image.fromarray(np.uint8(uploaded_file))
...
# Load u image
uploaded_file_one = st.sidebar.file_uploader("Выберите первое изображение", type=["jpg","png","JPEG"]) # добавить форматы файлов
